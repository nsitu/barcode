...
const scan = async () => {
  ...
  try {
    ...
    if (detections.length) { const detected = detections[0]; result.textContent = detected.rawValue; format.textContent = detected.format.replaceAll('_', ' ').toUpperCase(); setStatus('FOUND') }
  } catch (error) { console.error('Barcode detection failed:', error) }
  if (scanning) requestAnimationFrame(scan)
}

startButton.addEventListener('click', async () => {
  if (scanning) { scanning = false; stream?.getTracks().forEach((track) => track.stop()); video.srcObject = null; placeholder.hidden = false; cameraFrame.classList.remove('active'); startButton.textContent = 'START CAMERA'; setStatus('OFF'); return }
  try {
